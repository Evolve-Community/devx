...
class TestMain(TestCase):
    def setUp(self):
        ...
        self.patcher = patch("devx.coders.base_coder.check_model_availability")
    ...
    def test_main_with_git_config_yml(self):
        ...
        Path(".devx.conf.yml").write_text("no-auto-commits: true\n")
        with patch("devx.main.Coder.create") as MockCoder:
            main([], input=DummyInput(), output=DummyOutput())
            _, kwargs = MockCoder.call_args
            assert kwargs["auto_commits"] is False

        Path(".devx.conf.yml").write_text("auto-commits: true\n")
        with patch("devx.main.Coder.create") as MockCoder:
            main([], input=DummyInput(), output=DummyOutput())
            _, kwargs = MockCoder.call_args
    ...
    def test_main_with_empty_git_dir_new_subdir_file(self):
        ...
        # Because devx will try and `git add` a file that's already in the repo.
