...
To address these issues, `devx` now 
...
[chat transcript](https://evolve.network/devx/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [devx](https://github.com/evolve-community/devx#installation).
  - Install [universal ctags](https://github.com/universal-ctags/ctags).
  - Run `devx` inside your repo, and it should say "using ctags".
...
This works pretty well, and is supported by `devx` -- you
...
The latest version of `devx` sends a **repo map** to GPT along with
...
Here's a
sample of the map of the devx repo, just showing the maps of
[main.py](https://github.com/evolve-community/devx/blob/main/devx/main.py)
and
[io.py](https://github.com/evolve-community/devx/blob/main/devx/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `devx` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `devx` work
