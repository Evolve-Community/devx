...
class Commands:
    ...
    def cmd_undo(self, args):
        "Undo the last git commit if it was done by devx"
        if not self.coder.repo:
            self.io.tool_error("No git repository found.")
            return
        ...
        if (
            not last_commit.message.startswith("devx:")
            or last_commit.hexsha[:7] != self.coder.last_devx_commit_hash
        ):
            self.io.tool_error("The last commit was not made by devx in this chat session.")
            return
        self.coder.repo.git.reset("--hard", "HEAD~1")
        self.io.tool_output(
            f"{last_commit.message.strip()}\n"
            f"The above commit {self.coder.last_devx_commit_hash} "
    ...
    def cmd_diff(self, args):
        "Display the diff of the last devx commit"
        if not self.coder.repo:
            self.io.tool_error("No git repository found.")
            return

        if not self.coder.last_devx_commit_hash:
            self.io.tool_error("No previous devx commit found.")
            return

        commits = f"{self.coder.last_devx_commit_hash}~1"
        diff = self.coder.get_diffs(commits, self.coder.last_devx_commit_hash)
    ...
    def cmd_add(self, args):
        ...
        if self.coder.repo and git_added:
            ...
            commit_message = f"devx: Added {git_added}"
