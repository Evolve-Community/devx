...
from devx import models
from devx.coders import Coder
from devx.dump import dump  # noqa: F401
from devx.io import InputOutput

BENCHMARK_DNAME = Path(os.environ["DEVX_BENCHMARK_DIR"])
...
def main(
    ...
    no_devx: bool = typer.Option(False, "--no-devx", help="Do not run devx"),
    ...
    if "DEVX_DOCKER" not in os.environ:
...
def summarize_results(dirname):
    ...
    all_results = [json.loads(fname.read_text()) for fname in dirname.glob("*/.devx.results.json")]
...
def run_test(
    testdir, model_name, edit_format, tries, no_unit_tests, no_devx, verbose, commit_hash
):
    ...
    history_fname = testdir / ".devx.chat.history.md"

    results_fname = testdir / ".devx.results.json"
